//
//  RecentlyWornListController.swift
//  MyFitZ
//
...
class RecentlyWornListController: WKInterfaceController, WCSessionDelegate{
    ...
    var items: [WatchItem]?{
        didSet{
            ...
            self.table.setNumberOfRows(count, withRowType: RecentList)
            
            
            for index in 0..<self.table.numberOfRows{
                if let controller = table.rowControllerAtIndex(index) as? RecentlyWornListRowController{
