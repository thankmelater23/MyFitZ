...
class FavoritesListController: WKInterfaceController, WCSessionDelegate{
    ...
    var items: [WatchItem]?{
        didSet{
            ...
            self.table.setNumberOfRows(count, withRowType: FavList)
            
            
            for index in 0..<self.table.numberOfRows{
                if let controller = table.rowControllerAtIndex(index) as? FavoriteListRowController{
